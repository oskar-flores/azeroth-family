...
The script clones the playerbots fork (and refuses if the remote isn't it),
clones the module into `modules/`, verifies the module SQL is present, and builds
four images: `worldserver`, `authserver`, `db-import` (one compile, three
`--target`s against the core's own Dockerfile) and `llm-chatter-bridge`.
...
Pull upstream changes with `./build-and-push.sh --update`. Always update core and
module **together** — mixing versions gives compile errors or runtime crashes.
...
To update: `./build-and-push.sh --update`, then redeploy in Dokploy. Back up
first. The fork trails upstream AzerothCore, so third-party modules targeting the
newest AC may not compile — if one fails, find an older commit of it.
